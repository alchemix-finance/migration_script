...
#!/usr/bin/env python3
"""Phase A: Approve the underlying ERC20 (USDC/WETH) so the MYT vault can pull it.

Usage:
    ape run approve_underlying --chain arbitrum --asset eth --mode json
    ape run approve_underlying --chain arbitrum --asset eth --mode impersonate

Run BEFORE Phase B (`ape run deposit_myt`). Allowance target = total underlying
needed to mint enough MYT to back every position on this chain/asset.
"""
...
def cli(cli_ctx, chain: str, asset: str, mode: str, yes: bool, dry_run: bool, verbose: bool) -> None:
    ...
    underlying = myt_info["underlying"]
    underlying_symbol = myt_info["underlying_symbol"]
    ...
    click.echo(f"Underlying: {underlying_symbol} ({underlying}) decimals={underlying_decimals}")
    ...
    click.echo(click.style("\n[1/3] Validating CSV & computing total underlying...", fg="cyan", bold=True))
    ...
    click.echo(f"  Positions:          {result.total_positions}")
    click.echo(f"  Total MYT needed:   {result.total_deposit_wei:,} wei (18d)")
    click.echo(f"  Total underlying:   {total_underlying:,} ({underlying_symbol}, {underlying_decimals}d)")
    ...
    # Preflight: skip if allowance already sufficient OR if deposits already
    # complete (allowance consumed → 0 but work is done).
    try:
        from src.preflight import check_approve_underlying_done, check_deposit_done
        ...
        if dep_done:
            ...
            return
        done, msg, _current = check_approve_underlying_done(
            chain=chain, underlying=underlying, multisig=multisig, spender_myt=myt, required=total_underlying,
        )
    ...
    batches = create_approve_underlying_batches(
        underlying_address=underlying, myt_address=myt, amount_wei=total_underlying, chain=chain,
    )
    ...
    if not yes and not click.confirm(
        f"Proceed with {mode} on {chain} {asset_type.value}?"
    ):
